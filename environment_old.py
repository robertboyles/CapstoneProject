...
class Environment():
    ...
    def __init__(self, vehicleModel : BicycleModel, track : TrackDefinition) -> None:
    ...
    def  ode_fun(self, t, y):
            ...
            # if self.integrator is not None and self.integrator.t_old is not None:
            #     self.h = self.integrator.t - self.integrator.t_old
            # else:
            #     self.h = 1e-6
    ...
    def step(self, rBrakeThrottle, aHandWheel):
        ...
        self.time.append(self.integrator.t)
...
if __name__ == "__main__":
    ...
    while env.t < 255:
        ...
        if env.integrator.status == 'running':
            env.step(rBT, aSteer)
        else:
            break
